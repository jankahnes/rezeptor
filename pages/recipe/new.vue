...
const submitFromNaturalLanguage = async (recipe: BaseRecipe) => {
  ...
  job.value = await createJob(supabase, 'natural-language');
  setTimeout(() => {
...
const submitFromPreparsed = async (recipe: ComputableRecipe) => {
  currentView.value = 'loading';
  job.value = await createJob(supabase, 'preparsed');
...
const submitFromLink = async (link: string) => {
  ...
  if (supportedVideoSites.some((site) => link.includes(site))) {
    job.value = await createJob(supabase, 'media');
    setTimeout(() => {
    ...
    job.value = await createJob(supabase, 'link');
    setTimeout(() => {
...
const submitFromPicture = async (file: File) => {
  currentView.value = 'loading';
  job.value = await createJob(supabase, 'picture');
  setTimeout(() => {
