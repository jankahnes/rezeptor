...
export const useRecipeStore = defineStore('recipe', () => {
  ...
  const socialIndexRecipes = ref<RecipeOverview[]>([]);
  const isEditingNew = ref(false);
  const cachedSocialPictures = ref<Record<string, string | null>>({});
  let lastInstagramFetch = 0;
  const INSTAGRAM_DELAY = 2000; // 2 seconds between Instagram requests

  async function getSocialPicture(
    source: string,
    ...
    if (/instagram\.com/.test(source)) {
      const now = Date.now();
      const timeSinceLastFetch = now - lastInstagramFetch;
      if (timeSinceLastFetch < INSTAGRAM_DELAY) {
        await new Promise((resolve) =>
          setTimeout(resolve, INSTAGRAM_DELAY - timeSinceLastFetch)
        );
      }
  ...
  async function setSocialIndexRecipes(recipes: RecipeOverview[]) {
    socialIndexRecipes.value = recipes;
  }
