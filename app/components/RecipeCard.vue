<template>
  ...
  >
    <NuxtImg
      v-if="recipe?.picture"
      class="w-[85%] aspect-square object-contain relative z-10 shadow-gray-200 [filter:drop-shadow(0_0_8px_var(--tw-shadow-color))_drop-shadow(0_0_4px_var(--tw-shadow-color))] transition-all duration-300 group-hover:translate-y-[-1px] group-hover:scale-[1.008]"
      :src="recipe?.picture"
    />
    <div
      v-else-if="recipe?.social_picture"
      class="relative w-[85%] z-10 transition-all duration-300 group-hover:translate-y-[-2px] group-hover:scale-[1.008]"
    >
      <div
    ...
    <div
      class="w-full relative pt-[55%] z-0 transition-all duration-300 group-hover:translate-y-[1px] bg-primary-10 px-6  pb-[5%] rounded-3xl min-h-40 max-h-100"
      :class="{ '-mt-[50%] ': recipe.picture || recipe.source }"
    >
