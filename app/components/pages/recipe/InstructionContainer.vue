  ...
  >
    <div class="p-2 md:p-6 !pb-3" v-if="!hideHeader">
      <div class="flex justify-between items-center w-full mb-2">
        <div
          class="px-4 py-1 bg-primary-20 rounded-lg"
          v-if="!hideHeader"
        >
    ...
    <div class="flex-1 px-2" v-if="instructions && instructions.length > 0">
      <div class="max-w-2xl">
        ...
        >
          <div
            class="min-w-9 h-9 p-1 rounded-lg flex items-center text-xl font-bold justify-center bg-primary-20 text-gray-700 border border-gray-200 flex-shrink-0 mt-1"
          >
            {{ index + 1 }}
          </div>
    ...
    <transition name="fade" mode="out-in">
      <Teleport to="body" v-if="activeTooltip">
        ...
        >
          <div
            class="bg-white text-gray-700 px-3 py-1 rounded-lg shadow-lg text-sm font-medium text-center border border-gray-200"
          >
...
function renderInstructionWithLinks(instruction: string): string {
  ...
  return escaped.replace(
    ...
    (match, ingredient, ingredientId) => {
      return `<span 
      class="cursor-pointer relative ingredient-link px-0.5 bg-primary-20 rounded-md hover:bg-primary-20 transition-colors duration-400" 
