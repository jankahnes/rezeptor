<template>
  ...
  >
    <div class="flex flex-col items-start">
      <div v-if="!headless">
        <ComponentHeader title="NUTRITION INFO" />
        <p
      ...
      <div
        class="px-4 py-2 bg-primary-300 rounded-2xl text-xl font-bold"
        :class="headless ? 'mt-0' : 'mt-6'"
      >
        {{
          ...
          >
            <template v-if="item.key === 'connector'">
              <div class="relative h-4 w-full">
                <div
                  class="absolute h-full bg-gradient-to-b from-primary-200 to-primary-100 top-0 left-[calc((100%-2rem)/6)] w-[8px] pillar-clip-path"
                ></div>
                <div
                  class="absolute h-full bg-gradient-to-b from-primary-200 to-primary-100 top-0 right-[calc((100%-2rem)/6)] w-[8px] pillar-clip-path"
                ></div>
              </div>
            </template>
...
function getItemClass(item: NutritionalItem, row: NutritionalItem[]) {
  ...
  if (['carbohydrates', 'protein', 'fat'].includes(item.key)) {
    return 'px-4 py-2 bg-primary-200 rounded-2xl';
  } else if (['sugar', 'saturated_fat'].includes(item.key)) {
    return 'px-4 py-2 bg-primary-100 rounded-2xl';
  }
  return 'px-4 py-2 bg-primary-50 rounded-2xl';
}
