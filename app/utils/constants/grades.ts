const singleCategoryGrades = [
  { min_score: -300, grade: 'F' },
  { min_score: 4, grade: 'E-' },
  { min_score: 9, grade: 'E' },
  { min_score: 12, grade: 'E+' },
  { min_score: 15, grade: 'D-' },
  { min_score: 20, grade: 'D' },
  { min_score: 30, grade: 'D+' },
  { min_score: 38, grade: 'C-' },
  { min_score: 44, grade: 'C' },
  { min_score: 50, grade: 'C+' },
  { min_score: 56, grade: 'B-' },
  { min_score: 60, grade: 'B' },
  { min_score: 64, grade: 'B+' },
  { min_score: 68, grade: 'A-' },
  { min_score: 73, grade: 'A' },
  { min_score: 83, grade: 'A+' },
  { min_score: 88, grade: 'S-' },
  { min_score: 92, grade: 'S' },
  { min_score: 96, grade: 'S+' },
];

const ovrGrades = [
  { min_score: -300, grade: 'F' },
  { min_score: -2, grade: 'E-' },
  { min_score: 8, grade: 'E' },
  { min_score: 18, grade: 'E+' },
  { min_score: 23, grade: 'D-' },
  { min_score: 30, grade: 'D' },
  { min_score: 36, grade: 'D+' },
  { min_score: 41, grade: 'C-' },
  { min_score: 45, grade: 'C' },
  { min_score: 49, grade: 'C+' },
  { min_score: 53, grade: 'B-' },
  { min_score: 57, grade: 'B' },
  { min_score: 61, grade: 'B+' },
  { min_score: 65, grade: 'A-' },
  { min_score: 69, grade: 'A' },
  { min_score: 73, grade: 'A+' },
  { min_score: 78, grade: 'S-' },
  { min_score: 83, grade: 'S' },
  { min_score: 90, grade: 'S+' },
];

export const gradeColors: Record<string, string> = {
  S: 'bg-blue-100 text-blue-800',
  'S+': 'bg-blue-100 text-blue-800',
  'S-': 'bg-blue-100 text-blue-800',

  'A+': 'bg-emerald-200 text-emerald-800',
  A: 'bg-emerald-200 text-emerald-800',
  'A-': 'bg-emerald-200 text-emerald-800',

  'B+': 'bg-green-100 text-green-800',
  B: 'bg-green-100 text-green-800',
  'B-': 'bg-green-100 text-green-800',

  'C+': 'bg-yellow-100 text-yellow-800',
  C: 'bg-yellow-100 text-yellow-800',
  'C-': 'bg-yellow-100 text-yellow-800',

  'D+': 'bg-orange-200 text-orange-900',
  D: 'bg-orange-200 text-orange-900',
  'D-': 'bg-orange-200 text-orange-950',

  'E+': 'bg-red-200 text-red-900',
  E: 'bg-red-200 text-red-900',
  'E-': 'bg-red-200 text-red-900',

  F: 'bg-red-300 text-red-950',
};

export const gradeStickerColors: Record<string, string> = {
  S: 'sticker-blue text-blue-900 border-2 border-white/40',
  A: 'sticker-emerald text-emerald-900 border-2 border-white/40',
  B: 'sticker-green text-green-900 border-2 border-white/40',
  C: 'sticker-yellow text-yellow-900 border-2 border-white/40',
};

export const gradeTextColors: Record<string, string> = {
  S: 'text-blue-800',
  'S+': 'text-blue-800',
  'S-': 'text-blue-800',

  'A+': 'text-emerald-800',
  A: 'text-emerald-800',
  'A-': 'text-emerald-800',

  'B+': 'text-green-800',
  B: 'text-green-800',
  'B-': 'text-green-800',

  'C+': 'text-yellow-800',
  C: 'text-yellow-800',
  'C-': 'text-yellow-800',

  'D+': 'text-orange-900',
  D: 'text-orange-900',
  'D-': 'text-orange-900',

  'E+': 'text-red-900',
  E: 'text-red-900',
  'E-': 'text-red-900',

  F: 'text-red-950',
};

export const gradeBorderColors: Record<string, string> = {
  S: 'border-blue-100',
  A: 'border-green-200',
  B: 'border-green-200',
  C: 'border-yellow-100',
  D: 'border-orange-200',
  E: 'border-red-200',
  F: 'border-red-300',
};

export function getGradeBorderColor(
  score: number,
  type: 'single' | 'ovr' = 'single'
) {
  const grade = getGrade(score, type);
  const roundedGrade = grade.replace('+', '').replace('-', '');
  return gradeBorderColors[roundedGrade];
}

export function getGrade(
  score: number | null | undefined,
  type: 'single' | 'ovr' = 'single'
) {
  if (score == null) return '';
  let highest_grade = '';
  for (const grade of type === 'ovr' ? ovrGrades : singleCategoryGrades) {
    if (grade.min_score <= score) {
      highest_grade = grade.grade;
    }
  }
  return highest_grade;
}
