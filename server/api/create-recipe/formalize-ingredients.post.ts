...
export default defineEventHandler(
  async (event): Promise<{ ingredients: any[]; notes: string[] }> => {
    ...
    const { base_ingredients, recipe_context_string, jobId } = input;
    ...
    if (!formalizeResponse)
      throw new Error('No content returned from formalize response');
    ...
    for (const ingredient of processedIngredients) {
      await supabase
        .from('jobs')
        .update({
          message: `Looking at ${ingredient.name_original} 🔍`,
      ...
      try {
        ...
        if (response?.data?.action_taken === 'reject') {
          ingredient.id = null;
          notes.push(
            `${ingredient.name_original} rejected by food request system.`
          );
        } else {
          ...
          if (
            ...
            try {
              ...
              console.error(
                `Error reconciling unit for ${ingredient.name_original}:`,
                unitError
              );
              ingredient.unit = 'free';
              ingredient.amount = 0;
              notes.push(
                `${ingredient.name_original} with unit ${ingredient.unit} caused an error in the unit reconciliation system.`
              );
            }
          }
        }
        ...
        console.error(
          `Error processing ingredient ${ingredient.name_original}:`,
          error
        );
        ingredient.id = null;
        notes.push(
          `${ingredient.name_original} caused an error in the food request system.`
        );
      }
    }
