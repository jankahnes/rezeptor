...
export default defineEventHandler(
  async (event): Promise<{ unit_name: string }> => {
    ...
    const countableUnits = (food.countable_units || {}) as Record<
      string,
      number
    >;
    ...
    const directMatch = unitKeys.find(
      (key) => key.toLowerCase() === processedUnit.toLowerCase()
    );
    ...
    const pluralMatch = unitKeys.find((key) => {
      ...
      return (
        keyLower === singularizeWord(unitLower) ||
        keyLower === pluralizeWord(unitLower)
      );
    });
    ...
    const gptResponse = await $fetch('/api/gpt/response', {
      ...
      body: {
        message: prompt
          .replace(
            '{food_name}',
            foodName?.name || foodName?.food?.primary_name
          )
    ...
    if (result.action === 'match') {
      ...
      const finalUnitName =
        result.unit_name === 'whole' ? '' : result.unit_name;
    ...
    if (result.action === 'create') {
      ...
      if (updateError) {
        throw new Error(
          `Error updating countable units: ${updateError.message}`
        );
      }
