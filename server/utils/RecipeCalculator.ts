import {
  ...
  export default class RecipeCalculator {
    ...
    async computeRecipe() {
      ...
      this.recipeComputed.total_weight = Math.round(this.recipeComputed.total_weight);
    ...
    getCumulCols(ingredients) {
      ...
      this.recipeComputed.total_weight = 0;
      ...
      if (intrinsicSaltPer100g < this.gptTargetSalt) {
        const targetSaltTotal = (this.gptTargetSalt * this.recipeComputed.total_weight) / 100;
      ...
      for (const col in this.cumulColsComputed) {
        this.cumulColsComputed[col].amountPer100 =
          (this.cumulColsComputed[col].amountTotal /
            this.recipeComputed.total_weight) *
          100;
      }
      ...
      for (const ingredient of ingredients) {
        ...
        for (const col in this.cumulColsComputed) {
          ...
          const ingredientContributionPer100 =
            (ingredientContribution / this.recipeComputed.total_weight) * 100;
      ...
      this.cumulColsComputed['salt'] = {
        ...
        amountPer100: (this.cumulColsComputed.salt.amountTotal / this.recipeComputed.total_weight) * 100,
        amountPer100AfterAlpha: (this.cumulColsComputed.salt.amountTotal / this.recipeComputed.total_weight) * 100,
      ...
      if(this.recipeComputed.total_weight > 1500 || this.cumulColsComputed.kcal.amountTotal > this.unusualKcalThreshold) {
    ...
    getMNIDX() {
      ...
      for (const nutrient of micronutrient_weights) {
        ...
        const perServingContribution =
          perGramContribution * (this.recipeComputed.total_weight / 300);
        ...
        if (this.logToReport && perGramContribution > 0) {
          micronutrientDetails.push({
            ...
            rdaPerServing: Math.round(
              (rawValue * this.recipeComputed.total_weight) / rda
            ),
    ...
    async getSIDX() {
      ...
      return this.scale_by_points(prediction.prediction, [
        [10, 0],
        [40, 50],
        [50, 72],
        [60, 80],
    ...
    generateReport() {
      ...
      this.report.overall = {
        ...
        total_weight: Math.round(this.recipeComputed.total_weight),
      ...
      this.recipeComputed.report = this.getTrimmedReport();
      this.recipeComputed.fullReport = this.report;
    }

    getTrimmedReport() {
        const trimmedMicronutrients = this.report.micronutrients.details.map((nutrient) => ({
            name: nutrient.name,
