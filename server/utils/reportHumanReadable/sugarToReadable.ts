...
const totalSugarPer100Thresholds = {
    0: {description: "Minimal", ...generics.GREAT},
    1: {description: "Very low", ...generics.GREAT},
    2: {description: "Low", ...generics.GREAT},
    3: {description: "Moderately low", ...generics.GOOD},
    4: {description: "Moderate", ...generics.OKAY},
...
const naturalSourceThresholds = {
    ...
    0.95: {description: "All sugar from natural sources", ...generics.GREAT},
}
...
export default function sugarToReadable(report: any, isFood: boolean) {
    ...
    const naturalSourceItem = generics.getHighestThreshold(report.sugar.percentContributedFromNaturalSources, naturalSourceThresholds)
    ...
    items.push({
        ...
        subtitle: (report.sugar.percentContributedFromNaturalSources * 100).toFixed(0) + "%"
    })
    items.push({
        ...percentOfKcalItem,
        description: percentOfKcalItem.description + " impact on calories from sugar",
        subtitle: (report.sugar.percentOfKcal * 100).toFixed(0) + "%"
    })
    if(!isFood) {
    const contributors = report.contributors["sugar"]?.contributors || []
    for(const contributor of contributors) {
        if(contributor.totalContribution > 10) {
            items.push({
