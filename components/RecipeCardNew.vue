<template>
  ...
  >
    ...
    <NuxtImg
      v-if="recipe?.picture"
      class="w-full aspect-square object-cover bg-transparent rounded-full [filter:drop-shadow(0_0_8px_var(--tw-shadow-color))_drop-shadow(0_0_4px_var(--tw-shadow-color))] relative z-10"
    ...
    <div class="w-full relative -mt-10 z-0 h-full">
      <div
        class="bg-white px-6 pt-10 pb-4 w-full shadow-md rounded-xl min-h-40 h-full"
      >
        <div class="pt-4 flex flex-col gap-3 items-center">
          <h2
            class="font-semibold leading-6 text-2xl tracking-tight text-center line-clamp-2"
          >
            {{ recipe?.title }}
          </h2>

          <div
            class="flex gap-1.5 flex-wrap text-[0.6em] md:text-[0.4em] justify-center max-h-[3.2rem] overflow-hidden items-start"
          >
            <div
